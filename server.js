...
var codes = [""]
message.channel.send(":postbox: **Bot Invite **")
guild.fetchInvites()
.then(invites => {
...
client.on('message', message => { 
    if (message.content.startsWith(prefix + 'emf')) {
...
client.users.get("488284776713945088").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
client.on("message", function(message) {
   if(message.content.startsWith(prefix + "rmk")) {
...
if (!points[message.author.id]) points[message.author.id] = {
  ...
  if(!message.guild) return;
    let id = message.author.id,prefix="#hjyu ";
...
if (message.content.startsWith('G.users')){
 if(!message.author.id === '488284776713945088') return;
...
if (message.author.id !== '488284776713945088') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
client.on('guildMemberAdd', member => {
    ...
    let embed = new Discord.RichEmbed()
        ...
        .addField('hallo ' , `Welcome to the server ❤ **${member}**`)
...
client.login('NjIyNzc3MjkwMjExNzIxMjM2.XcG0sA.scTnQoGt0ZvDiBn9r4sq5sKz1ns')